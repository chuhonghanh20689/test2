							...
							<div class="row" style="margin-bottom:40px">
								<div class="col-xs-8">
									<div class="form-group" >
										...
										<input required type="text" name="pro_name" class="form-control" value="{{$product->pro_name}}">
									</div>
									<div class="form-group" >
										<label>Giá sản phẩm</label>
										<input required type="number" name="pro_price" class="form-control" value="{{$product->pro_price}}">
									</div>
									<div class="form-group" >
										<label>Ảnh sản phẩm</label>
										<input id="img" type="file" name="img" class="form-control hidden" onchange="changeImg(this)" >
					                    <img id="avatar" class="thumbnail" width="300px" src="{{asset('public/Product_upload/'.$product->img)}}">
									</div>
									<div class="form-group" >
									...
									<div class="form-group" >
										...
										<input required type="number" name="warranty" style="border:1px solid #e5e5e5" value="{{$product->warranty}}">tháng
									</div>
									<div class="form-group" >
									...
									<div class="form-group" >
										<label>Miêu tả</label>
										<textarea required name="des">{{$product->des}}</textarea>
									</div>
									<div class="form-group" >
										<label>Danh mục</label>
										<select required name="cat_id" class="form-control">
										@<?php foreach ($cat as $list): ?>
											<option value="{{$list->id}}" @if($product->cat_id==$list->id) {{'selected'}} @endif>{{$list->cat_name}}</option>
										<?php endforeach?>
					                    </select>
									</div>
									<div class="form-group" >
										<label>Số lượng nhập kho</label>
										<input required type="number" name="total_qtt" class="form-control" value="{{$product->total_qtt}}">
									</div>
									<div class="form-group" >
										<label>Số lượng tồn kho</label>
										<input required type="number" name="remaining_qtt" class="form-control" value="{{$product->remaining_qtt}}">
									</div>
									<div class="form-group" >
										<label>Sản phẩm nổi bật</label><br>
										Có:<input type="radio" name="hot" value="1" @if($product->hot==1) {{'checked'}} @endif>
										Không: <input type="radio" name="hot" value="0" @if($product->hot==0) {{'checked'}} @endif>
									</div>

			...
			<div id="wrap-inner" class="col-md-9">
				...
				<div class="row list-product">
					<div class="col-md-12">
						...
						<h3>Sản phẩm mới</h3>
						@foreach($pro_new as $list)
							<div class="col-xs-6 col-xs-offset-3 col-sm-offset-0 col-md-offset-0 col-sm-6 col-md-3 text-center post">
								<a href="{{asset('frontend/product/'.$list->id)}}"><img class="img-responsive" style="height:200px;" src="{{asset('public/product_upload/'.$list->img)}}"></a>
								<p><a href="{{asset('frontend/product/'.$list->id)}}">{{$list->pro_name}}</a></p>
								<span>{{$list->pro_price}} đ</span>
								<div class="hover col-xs-12 col-sm-12 col-md-12">
									<p><a href="{{asset('frontend/product/'.$list->id)}}">Xem chi tiết</a></p>
								</div>
							</div>
						@endforeach
					</div>
